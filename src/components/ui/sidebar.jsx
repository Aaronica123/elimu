...
const SidebarFooter = React.forwardRef(({ className, ...props }, ref) => (
  <div
    ref={ref}
    className={cn("mt-auto flex h-12 shrink-0 items-center gap-2 px-3", className)}
    {...props}
  />
));
SidebarFooter.displayName = "SidebarFooter";

const SidebarGroupContext = React.createContext({ heading: null });

const SidebarGroup = React.forwardRef(({ heading, className, ...props }, ref) => {
  return (
    <SidebarGroupContext.Provider value={{ heading }}>
      <div
        ref={ref}
...
const SidebarGroupHeading = React.forwardRef(({ className, ...props }, ref) => {
  const { heading } = React.useContext(SidebarGroupContext);
  return !heading ? null : (
    <h4
      ref={ref}
      className={cn(
        "mb-1 select-none px-3 text-xs font-medium uppercase text-muted-foreground/80 group-data-[state=collapsed]:hidden",
        className,
      )}
      {...props}
    >
...
SidebarGroupHeading.displayName = "SidebarGroupHeading";
